...
export default function ShareButtons({ emoji, english, korean, showToast }) {
  const { t } = useLang()
  const shareText = `${emoji} My result: ${t(english, korean)}\n\nTry free at glowmi.org!`
  ...
  function copyLink() {
    ...
    if (navigator.clipboard?.writeText) {
      navigator.clipboard.writeText(copyText).then(() => {
        showToast?.(t('Link copied!', '링크가 복사되었습니다!'))
      })
    } else {
  ...
  return (
    <div className="share-section">
      <p className="share-label">{t('Share Your Result', '결과 공유하기')}</p>
      <div className="share-buttons">
        <button className="share-btn share-btn-x" onClick={shareToTwitter} title={t('Share on X', 'X에 공유')}>
          <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
        </button>
        <button className="share-btn share-btn-fb" onClick={shareToFacebook} title={t('Share on Facebook', 'Facebook에 공유')}>
          <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </button>
        <button className="share-btn share-btn-copy" onClick={copyLink} title={t('Copy Link', '링크 복사')}>
        ...
        {typeof navigator !== 'undefined' && navigator.share && (
          <button className="share-btn share-btn-native" onClick={shareNative} title={t('Share', '공유')}>
